...
async function activate (externalProvider: ExternalProvider) {
  if (!externalProvider) { throw new Error('Failed to activate ethers: provider not found') }
  ...
  const getData = async (timeout = 5000) => {
    const timer = setTimeout(() => {
      throw new Error('Failed to activate ethers: timeout')
    }, timeout)
    ...
    clearTimeout(timer)
    return ([_network, _address, _balance])
  }
